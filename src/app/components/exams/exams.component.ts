...
export class ExamsComponent {
  ...
  filteredExams: ExamRequest[] = [...this.allExams];
  ...
  applyFilters() {
    this.filteredExams = this.allExams.filter(exam => {
      const matchesSearch = !this.searchTerm ||
        exam.familyMember.toLowerCase().includes(this.searchTerm.toLowerCase());

      const matchesDateRange = this.checkDateRange(exam.requestDate);
  ...
  saveNewExam() {
    ...
    // Gerar um novo ID para o exame
    const newId = 'REQ' + String(this.allExams.length + 1).padStart(3, '0');
  ...
  get paginatedExams(): ExamRequest[] {
